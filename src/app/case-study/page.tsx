...
export default function CaseStudyPage() {
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <SiteNav current="Case Study" />

      <article className="max-w-3xl mx-auto px-6 py-16">
        {/* Header */}
        <div className="mb-12">
          <div className="text-gray-800 text-xs uppercase tracking-widest font-semibold mb-4">Case Study</div>
        ...
        <div className="prose prose-invert prose-sm max-w-none space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={label} className="bg-white border border-gray-200 rounded-lg p-4">
                  <div className="text-gray-900 text-xs font-semibold mb-2">{label}</div>
                  <ul className="space-y-1 list-none p-0">
                    {items.map((item) => (
                      <li key={item} className="text-gray-600 text-xs flex items-start gap-2">
                        <span className="text-gray-500 mt-0.5">{label === 'Synth v5' ? '✓' : '✗'}</span>
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          <Section title="Architecture">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 font-mono text-xs text-gray-700 mb-4">
              <div className="text-gray-500 mb-2">┌─────────────────────────────────────────┐</div>
              <div className="text-gray-500">│  CLI Layer  (<span className="text-gray-800">src/cli/</span>)                   │</div>
              <div className="text-gray-500">│  analyze · demo · memo · revise · packet  │</div>
              <div className="text-gray-500">│  ingest · spreadsheet · dataroom (v4)     │</div>
              <div className="text-gray-500">│  triage · export · compare (v5)           │</div>
              <div className="text-gray-500">│  seed-demo · eval · pdf · doctor · verify │</div>
              <div className="text-gray-500">├─────────────────────────────────────────┤</div>
              <div className="text-gray-500">│  Library Layer  (<span className="text-gray-800">src/lib/</span>)              │</div>
              <div className="text-gray-500">│  ai-provider · mock-provider · parser     │</div>
              <div className="text-gray-500">│  spreadsheet-parser · mock-spreadsheet-provider (v4)│</div>
              <div className="text-gray-500">│  issue-engine · export-engine · compare-engine (v5)│</div>
              <div className="text-gray-500">│  html-renderer · pdf-writer · risk-scoring│</div>
              <div className="text-gray-500">├─────────────────────────────────────────┤</div>
              <div className="text-gray-500">│  Schema Layer  (<span className="text-gray-800">src/schemas/</span>)          │</div>
              <div className="text-gray-500">│  Zod: review · financial · memo · revision│</div>
              <div className="text-gray-500">│  TableProfile · SpreadsheetAnalysis (v4)  │</div>
              <div className="text-gray-500">│  DataRoomSummary · CrossDocumentFinding (v4)│</div>
              <div className="text-gray-500">│  Issue · EvidenceItem · IssueLog · CompareReport (v5)│</div>
              <div className="text-gray-500">├─────────────────────────────────────────┤</div>
              <div className="text-gray-500">│  UI Layer  (<span className="text-gray-800">src/app/</span>)                 │</div>
              <div className="text-gray-500">│  / · /demo · /artifacts · /dashboard      │</div>
              <div className="text-gray-500">│  /case-study                              │</div>
              <div className="text-gray-500">└─────────────────────────────────────────┘</div>
              <div className="mt-3 text-gray-500">          ↓ outputs to</div>
              <div className="text-gray-500">  reports/ (json · md · html · pdf · evals)</div>
              <div className="text-gray-500">  reports/issues/ · reports/evidence/ (v5)</div>
              <div className="text-gray-500">  reports/exports/ · reports/compare/ (v5)</div>
              <div className="text-gray-500">  public/demo-artifacts/ (static for /artifacts)</div>
              <div className="text-gray-500">  src/data/demo/ (fixtures for /demo)</div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={layer} className="bg-white border border-gray-200 rounded-lg p-4">
                  <div className="text-gray-900 font-medium text-sm mb-1">{layer}</div>
          ...
          <Section title="What Changed from v1 to v5">
            <div className="space-y-3">
              {[
                ...
                <div key={version} className="bg-white border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-mono text-gray-800 bg-white border border-gray-300 px-2 py-0.5 rounded">{version}</span>
          ...
          <Section title="How the Workflow Works">
            <div className="bg-white border border-gray-200 rounded-xl p-5 font-mono text-xs text-gray-700 space-y-1 mb-4">
              <div className="text-gray-500"># 1. Drop documents into inbox</div>
              <div>cp my-contract.pdf documents/inbox/</div>
              <div>cp cap-table.csv documents/inbox/</div>
              <div className="mt-2 text-gray-500"># 2a. Contract review pipeline</div>
              <div>npm run packet</div>
              <div className="pl-4 text-gray-800">→ Detect document type (.txt/.md/.pdf/.docx)</div>
              <div className="pl-4 text-gray-800">→ Run contract review + financial + memo + revision</div>
              <div className="pl-4 text-gray-800">→ Render HTML → PDF via Playwright</div>
              <div className="mt-2 text-gray-500"># 2b. v4: Spreadsheet + data room analysis</div>
              <div>npm run spreadsheet  <span className="text-gray-500"># CSV/XLSX → reports/tables/</span></div>
              <div>npm run dataroom     <span className="text-gray-500"># cross-doc → reports/dataroom/</span></div>
              <div className="mt-2 text-gray-500"># 2c. v5: Analyst workflow loop</div>
              <div>npm run triage       <span className="text-gray-500"># reports → issue log + evidence ledger</span></div>
              <div className="pl-4 text-gray-800">→ Convert all findings to structured Issues</div>
              <div className="pl-4 text-gray-800">→ Link every issue to at least one EvidenceItem</div>
              <div className="pl-4 text-gray-800">→ Save JSON + MD + HTML → reports/issues/ + reports/evidence/</div>
              <div className="mt-1">npm run export       <span className="text-gray-500"># issues + data → CSV/XLSX</span></div>
              <div className="pl-4 text-gray-800">→ issues.csv, evidence.csv, payments.csv</div>
              <div className="pl-4 text-gray-800">→ cap-table.csv, dataroom-summary.xlsx (5-sheet workbook)</div>
              <div className="mt-1">npm run compare      <span className="text-gray-500"># diff two runs</span></div>
              <div className="pl-4 text-gray-800">→ Added / removed / changed issues</div>
              <div className="pl-4 text-gray-800">→ Payment and cap table deltas</div>
              <div className="pl-4 text-gray-800">→ Save JSON + MD + HTML → reports/compare/</div>
              <div className="mt-2 text-gray-500"># 3. Open dashboard</div>
          ...
          <Section title="Safety and Limitations">
            ...
            <div className="space-y-3">
              {[
                { title: 'Visible disclaimer', desc: 'The "not legal advice" disclaimer appears exactly once on every page and rendered report — the site footer, each report\'s footer, and the cover fine print of printable documents — so the framing stays clear without drowning the content.' },
            ...
            <div className="bg-amber-50/30 border border-amber-200/40 rounded-lg p-4 mt-2">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              <ul className="space-y-2 list-none p-0">
                {[
                  ...
                  <li key={bullet} className="text-gray-700 text-xs flex items-start gap-2">
                    <span className="text-gray-500 mt-0.5 min-w-[8px]">›</span>
                    {bullet}
                  </li>
                ))}
              </ul>
          ...
          <div className="border-t border-gray-200 pt-8 mt-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/artifacts" className="border border-gray-300 hover:border-gray-400 text-gray-700 text-xs px-4 py-2 rounded transition-colors">
                Download Artifacts
              </Link>
              <a href="https://github.com/dcablayan/Synth" className="border border-gray-300 hover:border-gray-400 text-gray-700 text-xs px-4 py-2 rounded transition-colors">
                GitHub
              </a>
            </div>
          </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      ...
      <div className="space-y-4 text-gray-700 text-sm leading-relaxed [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-2 [&_ol]:space-y-2 [&_li]:text-gray-700 [&_p]:text-gray-700 [&_strong]:font-semibold">
        {children}
      </div>
    </section>
  );
