...
async function main() {
  ...
  const fullText = fs.readFileSync(sourceFile, 'utf-8');
  ...
  // Seed spreadsheet demo artifacts
  console.log('\n  Seeding v4 spreadsheet demo data...\n');
  try {
    ...
    writeFile(
      path.join(DEMO_ARTIFACTS, 'demo-dataroom.json'),
      JSON.stringify(dataRoomSummary, null, 2),
      'demo-dataroom.json (public)'
    );
  } catch (err) {
